...
describe 'Test Creating a Pull Request' do
  ...
  describe "#run" do
    ...
    it "running whole program" do
      expect(@generator).to receive(:create_or_update).exactly(4).times do |job, xml|
        expect(['pull_req_test-PR5-10','pull_req_test-PR5-11','pull_req_test-PR6-10','pull_req_test-PR6-11']).to include(job[:name])
      end
